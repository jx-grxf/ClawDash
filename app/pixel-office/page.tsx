...
function MiniSparkline({ data, width = 120, height = 24, color: fixedColor }: { data: number[]; width?: number; height?: number; color?: string }) {
  ...
  const hash = data.reduce((acc, value, index) => ((acc * 31 + Math.round(value) + index) >>> 0), 7)
  const id = `spark-${width}-${height}-${hash.toString(36)}`
  return (
...
export default function PixelOfficePage() {
  ...
  useEffect(() => {
    ...
    if (!externalFetchesEnabled) return

    fetch('/api/pixel-office/contributions')
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data && data.weeks && !data.disabled) {
          contributionsRef.current = data
        }
      })
      .catch(() => {})
  }, [externalFetchesEnabled])
  ...
  }, [forceEditorUpdate, layoutWriteEnabled])
                  ...
                  <div className="space-y-3">
                    ...
                    {info.disabled ? (
                      <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                        Externe Fetches sind aus. Es wird nur die lokale Information angezeigt.
                      </div>
                    ) : null}
