...
import { getDashboardAccessConfig, maskSensitiveValue } from "@/lib/dashboard-access";
import { resolveGatewayEndpoints } from "@/lib/gateway-url";
import { parseSessionKey, readSessionsIndex, type SessionsIndex } from "@/lib/openclaw-domain";
  ...
  for (const agentId of agentIds) {
    try {
      ...
      for (const key of Object.keys(sessions)) {
        ...
        if (parsed.transport !== "group" || !parsed.platform || !parsed.target) continue;
        const gid = `${parsed.platform}:${parsed.target}`;
        if (!groupAgents[gid]) groupAgents[gid] = { agents: new Set(), channel: parsed.platform };
        groupAgents[gid].agents.add(agentId);
        if (feishuAgentIds.includes(agentId) && parsed.platform === "feishu") {
          groupAgents[gid].channel = "feishu";
        }
      }
    } catch {}
  }
  ...
  for (const agentId of agentIds) {
    try {
      ...
      for (const [key, val] of Object.entries(sessions)) {
        const parsed = parseSessionKey(key);
        if (parsed.transport !== "direct" || parsed.platform !== "feishu" || !parsed.target) continue;
        const updatedAt = (val as any).updatedAt || 0;
        if (!best || updatedAt > best.updatedAt) {
          best = { openId: parsed.target, updatedAt };
        }
      }
...
function getChannelDirectPeerIds(
  ...
  for (const agentId of agentIds) {
    try {
      ...
      for (const [key, val] of Object.entries(sessions)) {
        const parsed = parseSessionKey(key);
        if (parsed.transport !== "direct" || parsed.platform !== channel || !parsed.target) continue;
        const updatedAt = (val as any).updatedAt || 0;
        if (!best || updatedAt > best.updatedAt) {
          best = { peerId: parsed.target, updatedAt };
        }
      }
...
export async function GET() {
  ...
  try {
    const accessConfig = getDashboardAccessConfig();
    ...
    agentList = agentList.filter((agent: any) => !HIDDEN_AGENT_IDS.has(String(agent?.id || "")));
    const agentIds = agentList.map((a: any) => a.id);
    const sessionsMap = new Map<string, SessionsIndex>();
    for (const agentId of agentIds) {
      try {
        sessionsMap.set(agentId, readSessionsIndex(agentId));
      } catch {}
    }
    ...
    const agents = await Promise.all(agentList.map(async (agent: any) => {
      ...
      if (feishuBinding) {
        ...
        addPlatform({
          ...
          accountId,
          appId,
          ...(userOpenId && { botOpenId: accessConfig.privacyMode ? maskSensitiveValue(userOpenId, 4, 2) : userOpenId }),
        });
      }
      ...
      if (id === "main") {
        ...
        for (const channelName of enabledChannelNames) {
          ...
          addPlatform({
            name: channelName,
            ...(botUserId && { botUserId: accessConfig.privacyMode ? maskSensitiveValue(botUserId, 4, 2) : botUserId }),
          });
        }
      }
    ...
    const data = {
      ...
      gateway: {
        ...
        token: accessConfig.privacyMode ? "" : (config.gateway?.auth?.token || ""),
      },
      groupChats,
      privacyMode: accessConfig.privacyMode,
    };
