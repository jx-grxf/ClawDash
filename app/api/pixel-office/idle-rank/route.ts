...
let cache: CacheEntry<{ agents: Array<{ agentId: string; onlineMinutes: number; activeMinutes: number; idleMinutes: number; idlePercent: number }> }> | null = null;
...
export async function GET() {
  try {
    const cached = getCachedData(cache, CACHE_TTL);
    if (cached) {
